...
const Suggestions = ({ Toggle }) => {
  ...
  const updateSuggestions = async (id) => {
    try {
      await fetch(`${BASE_URL}ask/updateAllSuggestionsAd`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          authorization: `Bearer ${localStorage.getItem("token")}`,
        },
        body: JSON.stringify({ isNewReplyUser: false }),
      });
    ...
    <div className="px-3">
      ...
      <div className="container-fluid">
        <div className="row bg-white g-3 my-2 pt-1 pb-2 mt-4 fw-bolder fs-2 rounded-3 shadow-sm">
          <h3 className="text-center text-dark">All Suggestions</h3>
        </div>
